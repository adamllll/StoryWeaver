...
1. **字数必须相近**：输出字数必须在目标范围内，不能大幅缩减
2. **内容必须完整**：原文的每一个情节、对话、描写都要保留
3. **只改表达方式**：换词、换句式、换视角，但不删减内容

## 重写风格说明

- **保持原意**：只优化语言，字数保持在原文的90%-110%
- **增强感染力**：增加情感渲染和细节，字数增加到原文的120%-150%
- **简化表达**：精简语言但保留所有内容，字数保持在原文的70%-90%
- **改变视角**：转换叙事视角，字数保持在原文的90%-110%
- **增加对话**：将叙述改为对话，字数增加到原文的130%-160%
- **增加描写**：补充环境、心理、动作描写，字数增加到原文的140%-180%
...
- 字数可能增加40%-80%""",
    ...
    user_prompt = REWRITE_USER_TEMPLATE.format(
        category=category,
        novel_description=novel_context[:300],  # 限制长度
...
```json
{{
    "content": "本章内容（800-1500字，Markdown格式）",
    "state_changes": {{
        "生命值": -20,
        "灵力": -30,
        "物品+": [
